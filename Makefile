...
BOOTROM_PATH ?= ./remote/bootrom.bin
WORKLOAD_PATH ?= ./remote/RV_BOOT.bin
BITSTREAM_PATH ?= ./remote/system.bit
...
.PHONY: all build_tools clean_tools list_devices program_fpga rescan_pci load_driver load_and_run init_usergroup fetch_remote_sw fetch_remote_hw fetch_remote
...
load_and_run:
	sudo ./scripts/load_and_run.sh xdma0 $(BOOTROM_PATH) $(WORKLOAD_PATH)
...
ifneq ($(PATH_TO_NEXST),unset)
	...
	bash -c "scp ${USER}@${EDA_HOST}:${PATH_TO_NEXST}/${REMOTE_SW_DIR}/{bootrom.bin,RV_BOOT.bin} ./remote/"
	@echo "Software files successfully fetched to ./remote"
else
...
ifneq ($(PATH_TO_NEXST),unset)
	...
	bash -c "scp ${USER}@${EDA_HOST}:${PATH_TO_NEXST}/${REMOTE_HW_DIR}/{debug_nets.ltx,system.bit,system.hdf} ./remote/"
	@echo "Hardware files successfully fetched to ./remote"
else
	$(error PATH_TO_NEXST is not set. Please specify path to Nexst build directory.)
endif
else
